...
class EpicApp:
    ...
    def __init__(self):
        global red_scale,blue_scale,green_scale, x_amp_distort_scale, y_amp_distort_scale, user_file_name_box, echo_decay_scale, echo_frames_scale, import_name_box, directory_box
        self.root = tk.Tk()
        ...
        echo_on_button = tk.Checkbutton(self.root, text="echo", font=('DevLys 010', 25), bg="lightblue",
                                           fg="darkblue", command=self.change_echo_state)
        echo_on_button.place(x=230, y=250)
        ...
        echo_decay_scale = tk.Scale(self.root, from_=1,to=0, resolution=0.1)
        echo_frames_scale = tk.Scale(self.root, from_=30,to=0)

        echo_decay_scale.place(x = 230, y = 100)
        echo_frames_scale.place(x = 270, y = 100)
        ...
        mp_label = tk.Label(self.root, text=".mp4", font=('DevLys 010', 25), bg="lightblue",
                                 fg="darkblue")
        mp_label.place(x=530, y=450)
        ...
        import_name_box = tk.Text(self.root, font=(18))
        import_name_box.place(width = 350, height= 35, x=250, y=350)

        directory_box = tk.Text(self.root, font=(18))
        directory_box.place(width=350, height=35, x=250, y=400)
    ...
    def import_file(self):
        ...
        import_name_box = tk.Label(self.root, text=self.root.filename, bg="darkblue",fg="white", font=(10))
        import_name_box.place(width = 350, height= 35, x=250, y=350)

    def select_directory(self):
        self.root.directory = filedialog.askdirectory(title = "choose where u want it saved")
        self.directory_selected = True
        directory_box = tk.Label(self.root, text = self.root.directory, bg="darkblue",fg="white", font=(10))
        directory_box.place(width=350, height=35, x=250, y=400)

    def change_video(self):
        if(self.file_selected and self.directory_selected):
            previous_frames = deque(maxlen=int(echo_frames_scale.get()))
        ...
        def apply_echo_effect(self,frame):
            ...
            for i, prev_frame in enumerate(previous_frames):
                echo_frame += (echo_decay_scale.get() ** i) * prev_frame
